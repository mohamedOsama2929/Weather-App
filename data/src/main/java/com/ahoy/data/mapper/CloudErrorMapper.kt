...
open class CloudErrorMapper @Inject constructor(private val gson: Gson) {
    ...
    fun mapToDomainErrorException(throwable: Throwable?): ErrorModel {
        val errorModel: ErrorModel? = when (throwable) {
            ...
            is HttpException -> {
                ...
                when {
                    throwable.code() == 401 -> ErrorModel(ErrorStatus.UNAUTHORIZED, 401)
                    throwable.code() == 500 -> ErrorModel(ErrorStatus.INTERNAL_SERVER_ERROR, 500)
                    throwable.code() == 403 -> getBadRequestHttpError(
                        throwable.response()?.errorBody()
                    )
                    throwable.code() == 404 -> ErrorModel(ErrorStatus.NOT_FOUND, 404)
                    throwable.code() == 400 -> getBadRequestHttpError(
                        throwable.response()?.errorBody()
                    )
                    throwable.code() == 422 -> ErrorModel(ErrorStatus.UNPROCESSABLE_ENTITY, 422)
            ...
            is SocketTimeoutException -> {
                ErrorModel("No internet connection!!", 0, ErrorStatus.TIMEOUT)
            }

            // handle connection error
            is UnknownHostException -> {
                ErrorModel("Server error", 0, ErrorStatus.UNKNOWN_HOST)
            }

            is IOException -> {
                ErrorModel("No internet connection", 0, ErrorStatus.NO_CONNECTION)
            }

            else ->
                ErrorModel("NOT DEFINED", 0, ErrorStatus.NOT_DEFINED)
        }
        return errorModel!!
    }

    fun mapToResponseErrorMessage(message: String): ErrorModel {
        return ErrorModel(message, 400, ErrorStatus.EMPTY_RESPONSE)
    }
    ...
    private fun getBadRequestHttpError(body: ResponseBody?): ErrorModel {
        return try {
            ...
            Log.d("getHttpError", "getErrorMessage() called with: errorBody = [$result]")
            val json = Gson().fromJson(result, JsonObject::class.java)
            ErrorModel(json["message"]?.asString ?: json.asString, 400, ErrorStatus.BAD_RESPONSE)
        } catch (e: Throwable) {
            e.printStackTrace()
            ErrorModel(ErrorStatus.NOT_DEFINED)
        }
