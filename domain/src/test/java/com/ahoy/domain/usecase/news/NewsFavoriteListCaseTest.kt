...
class NewsFavoriteListCaseTest {

    //system under test
    private lateinit var newsFavoriteListUseCase: NewsFavouriteListUsecase
    ...
    fun initEach() {
        ...
        mapper = NewsMapper()
        mockJson = MockJson()
        newsFavoriteListUseCase = NewsFavouriteListUsecase(
            errorUtil,
            appRepository,
            mapper
        )
    ...
    fun convertRemoteNewsDtoToNewsBo_returnNewsBo() = runBlocking {
        //Arrange
        val dto: NewsListResponse = mockJson.getFavouriteNewsList()
    ...
    fun executeOnBackground_returnNewsDetailsDto() = runBlocking {
        //Given
        val dto = mockJson.getFavouriteNewsList()
        val parameters = NewsListQuery(1, 10)

        //when
        mockitoWhen(appRepository.getFavouriteNewsList(parameters))
            .thenReturn(dto)

        val response = appRepository.getFavouriteNewsList(parameters)
        //then
