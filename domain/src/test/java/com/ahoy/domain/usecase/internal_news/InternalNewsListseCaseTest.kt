...
class InternalNewsListseCaseTest {

    //system under test
    private lateinit var newsListUseCase: InternalNewsListUsecase
    ...
    fun initEach() {
        ...
        mockJson = MockJson()
        newsListUseCase = InternalNewsListUsecase(
            errorUtil,
            appRepository,
            mapper
        )
    ...
    fun convertRemoteNewsDtoToNewsBo_returnNewsBo() = runBlocking {
        //Arrange
        val dto: InternalNewsListResponse = mockJson.getInternalNewsList()
    ...
    fun executeOnBackground_returnNewsListDto() = runBlocking {
        //Given
        val dto = mockJson.getInternalNewsList()
        val parameters = NewsListQuery(1, 10)

        //when
        mockitoWhen(appRepository.getInternalNewsList(parameters))
            .thenReturn(dto)

        val response = appRepository.getInternalNewsList(parameters)
