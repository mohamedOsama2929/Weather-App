...
class AnnouncementsFavouriteListCaseTest {

    //system under test
    private lateinit var announcementsListUseCase: AnnouncementsFavouriteListUsecase
    ...
    fun initEach() {
        ...
        mockJson = MockJson()
        announcementsListUseCase = AnnouncementsFavouriteListUsecase(
            errorUtil,
            appRepository,
            mapper
        )
    ...
    fun convertRemoteAnnouncementDtoToAnnouncementBo_returnAnnouncementBo() = runBlocking {
        //Arrange
        val dto: AnnouncementsListResponse = mockJson.getAnnouncementFavoritesList()
    ...
    @Test
    fun executeOnBackground_returnAnnouncementsFavoriteListDto() = runBlocking {
        //Given
        val dto = mockJson.getAnnouncementFavoritesList()
        val parameters = AnnouncementsListQuery(1, 10)

        //when
        mockitoWhen(appRepository.getFavouriteAnnouncementsList(parameters))
            .thenReturn(dto)

        val response = appRepository.getFavouriteAnnouncementsList(parameters)
